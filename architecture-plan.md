...
**MVP scoring UI scope (confirmed, revised):** the viewer experience is a TV-style score bug — score, inning, top/bottom, live/final status, **plus outs and the ball-strike count**. Outs and count are in scope for MVP (closer to an authentic broadcast feel); this means `games` needs `outs int`, `balls int`, `strikes int` columns (reset each half-inning/at-bat) alongside the existing `our_score`/`opponent_score`/`current_inning`/`inning_half`, and `record_score_event()` (or a sibling RPC) needs to handle ball/strike/out events, not just runs. Not yet built — this is scoped for whenever the live-scoring phase actually starts, not part of Phase 1 (Teams & Roster).

**Pitcher pitch count is explicitly excluded from MVP**, regardless of the above — it requires tracking who is currently pitching (not modeled anywhere yet: `game_lineup` has positions but no "current pitcher for this at-bat" concept), which is real added data-modeling work tied to the detailed engine below, not the simple scoreboard.

A GameChanger-style detailed live scoring engine — pitch-by-pitch, at-bat outcomes, full box score, play-by-play feed, pitcher pitch count — is explicitly **post-MVP** (see §8a). It's a meaningfully larger scope (new tables for at-bats/pitches, a real scoring-rules engine) and would slow down getting the MVP scoreboard live. Build the simple version (now including outs/count) first, layer in full detail later without needing to break the `games`/`game_score_events` model that already exists.
